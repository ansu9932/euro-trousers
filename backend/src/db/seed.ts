...
export const ROLE_NAMES=['System Administrator','Customs Manager','Documentation Officer','Data Entry Officer','Warehouse Officer','Finance Officer','Logistics Officer','General Manager','Viewer','Auditor'] as const
...
const roleRules:Record<string,{modules:string[];actions:string[]}>={
...
for(const name of ROLE_NAMES)await db.insert(roles).values({name}).onConflictDoNothing()
for(const module of modules)for(const action of actions)await db.insert(permissions).values({module,action}).onConflictDoNothing()
const allRoles=await db.select().from(roles),allPermissions=await db.select().from(permissions)
for(const role of allRoles){const rule=roleRules[role.name];if(!rule)continue;for(const permission of allPermissions.filter(p=>rule.modules.includes(p.module)&&rule.actions.includes(p.action)))await db.insert(rolePermissions).values({roleId:role.id,permissionId:permission.id}).onConflictDoNothing()}
...
for(const code of INCOTERMS_2020)await db.insert(masterRecords).values({type:'INCOTERM',code,nameEn:code}).onConflictDoNothing()
...
for(const code of ['AED','USD','EUR','GBP'])await db.insert(masterRecords).values({type:'CURRENCY',code,nameEn:code}).onConflictDoNothing()
for(const code of ['PCS','KG','MTR','ROLL','CTN'])await db.insert(masterRecords).values({type:'UOM',code,nameEn:code}).onConflictDoNothing()
...
if(!passwordSchema.test(password))throw new Error('BOOTSTRAP_ADMIN_PASSWORD must be 12+ chars with upper, lower, number, and symbol')
const [adminRole]=await db.select().from(roles).where(eq(roles.name,'System Administrator')).limit(1)
if(adminRole)await db.insert(users).values({email,passwordHash:await hashPassword(password),displayName:'System Administrator',roleId:adminRole.id}).onConflictDoNothing()
await pool.end()
